...
# Load the dataset
train_dataset =voc.VOC('train', common_transform=util.CommonTransforms())
...
optimizer = torch.optim.Adam(fcn_model.parameters(), lr=1e-4, weight_decay=1e-5)
...
# Save the best model
save_path = 'models/4c_best_model.pth'
...
def train():
    ...
    for epoch in range(epochs):
        ...
        if early_stop > early_stop_epoch:
            ...
            break
            torch.save(fcn_model.state_dict(), "models/end_model.pth")
    ...
    with torch.no_grad(): # we don't need to calculate the gradient in the validation/testing
        ...
        for _, (inputs, label) in enumerate(val_loader):
            ...
            for p, t in zip(pred, target): 
                ious.append(util.iou(p, t)) # 1 X H X W
